...
type Temperature = 'Treasure!' | 'Boiling hot!' | 'Really hot' | 'Hot' | 'Warm' | 'Cold' | 'Really cold' | 'Freezing!';
...
function getRandomPoint({ right, bottom }: { right: number, bottom: number }): Point {
    return {
...
function getTemperature(a: Point, b: Point, zoom = 1): Temperature {
    return getDistanceHint(getDistance(augmentPoint(a, zoom), augmentPoint(b, zoom)));
}
