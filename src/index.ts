...
import platformGame from "./platform-game-dom";
...
function createHref(textContent: string) {
  return `/${textContent.toLowerCase().replace(/ /g, "-")}`;
}
...
switch (location.pathname) {
  ...
  case createHref(routeNames[5]):
    renderPage(snake);
    break;
  case createHref(routeNames[6]):
    renderPage(platformGame);
    break;
}
